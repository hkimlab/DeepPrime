...
def main():
    ...
    ## Parameters
    sANALYSISTAG = 'deepprime_input_example'
    ...
    dict_params = { 'nAltIndex' : 60,        # 60nts --- Alt --- 60nts *0-based
                    ...
                    'Cell_line' : 'HEK293T'  # HEK293T, HCT116, MDA-MB-231, DLD1, A549, HeLa, NIH3T3 
                    }
    ...
    for nStart, nEnd in list_nBins:
        ...
        list_sParameters.append([sANALYSISTAG, df_sSubSplits, dict_params])
...
def mp_processor(list_sParameters: list, sBase_DIR=os.getcwd()):
    sANALYSISTAG, df_sSubSplits, dict_params = list_sParameters
    ...
    df_stat.to_csv('%s/%s_%s_%d_stat.csv' % (out_dir, sANALYSISTAG, dict_params['PE_system'], idx), index=False)
    df_top4_lib.to_csv('%s/%s_%s_AA_top4_lib.csv' % (out_dir, sANALYSISTAG, dict_params['PE_system']), index=False)
