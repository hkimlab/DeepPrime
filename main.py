...
def main(input_file, sample_name, pe_type, cell_type, pbs_min, pbs_max):
    ...
    for nStart, nEnd in list_nBins:
        ...
        list_sParameters.append([sample_name, df_sSubSplits, dict_params, nStart, nEnd])
...
def mp_processor(list_sParameters: list):
    sBase_DIR=os.getcwd()
    sample_name, df_sSubSplits, dict_params, nStart, nEnd = list_sParameters
    out_dir = '%s/output/%s/%s' % (sBase_DIR, sample_name, dict_params['PE_system'])
    ...
    for idx in df_sSubSplits.index:
        ...
        result, DPStop4 = deep_prime(sample_name, sID, Ref_seq, ED_seq, sAlt, dict_params)
    ...
    df_top4_lib.to_csv('%s/%s_%s_%d-%d_AA_top4_lib.csv' % (out_dir, sample_name, dict_params['PE_system'], nStart, nEnd), index=False)
...
def deep_prime(sample_name: str,
    ...
    sOut_DIR = '%s/output/%s/%s/results' % (sBase_DIR, sample_name, pe_system)
...
if __name__ == '__main__':
    if len(sys.argv) == 1:
        ...
        parser.add_argument("-f", "--file", type=str, help="Input file containing target sequence and edit type")
        parser.add_argument("-n", "--name", type=str, help="Sample name for your input", default='Sample')
        parser.add_argument("-p", "--pe_type", type=str, help="PE type parameter", default='PE2max')
        parser.add_argument("-c", "--cell_type", type=str, help="Cell type parameter", default='HEK293T')
        parser.add_argument("--pbs_min", type=int, help="PBS minimum length parameter", default=1)
        parser.add_argument("--pbs_max", type=int, help="PBS maximun length parameter", default=17)
        ...
        main(input_file, sample_name, pe_type, cell_type, pbs_min, pbs_max)
